...
row.open=true;
row.innerHTML='<summary class="span-summary"><span class="span-summary-title">Vano</span><span class="span-summary-meta"></span></summary><div class="span-body"><div class="span-head"><div class="field"><label>ID vano</label><input type="text" class="span-id" placeholder="S1" /></div><div class="field"><label>Vano sismico</label><input type="text" class="span-seismic" placeholder="190" /></div><div class="field"><label>Vano gravedad</label><input type="text" class="span-gravity" placeholder="190" /></div><div class="field"><label>Fraccion d/h</label><input type="number" class="span-d-ratio" step="0.01" min="0.01" max="1" placeholder="0.90" /></div><div class="field"><label>Barra longitudinal</label><select class="span-longitudinal-bar"><option>#3</option><option>#4</option><option>#5</option><option>#6</option><option>#7</option><option>#8</option><option>#9</option><option>#10</option><option>#11</option></select><p class="help">Mismo diámetro superior e inferior; solo contexto de detallado.</p></div><div class="field"><label><input type="checkbox" class="span-compression-rebar-required" /> Existe refuerzo longitudinal a compresión requerido</label><p class="help">Marcar únicamente cuando las barras longitudinales se contabilizan como refuerzo a compresión requerido por diseño.</p></div><div class="field"><label>Apoyo izq (mm)</label><input type="number" class="span-support-left" step="1" min="0" placeholder="0" /></div><div class="field"><label>Apoyo der (mm)</label><input type="number" class="span-support-right" step="1" min="0" placeholder="0" /></div><div class="field"><label>L libre real (mm)</label><input type="number" class="span-clear-length" step="1" min="1" placeholder="auto ETABS" /></div><div class="field"><label>Accion</label><button type="button" class="mini-btn mini-btn-danger span-remove">Eliminar vano</button></div></div><div class="region-box"><div class="region-head"><strong>Regiones del vano</strong><div class="region-actions"><button type="button" class="mini-btn btn-regenerate-regions">Regenerar 3 regiones base</button><button type="button" class="mini-btn btn-add-region">Agregar region</button></div></div><p class="help">Las regiones deben cubrir continuo de 0.0 a 1.0.</p><div class="region-list"></div></div></div>';
const idIn=row.querySelector('.span-id'),sIn=row.querySelector('.span-seismic'),gIn=row.querySelector('.span-gravity'),drIn=row.querySelector('.span-d-ratio'),lbIn=row.querySelector('.span-longitudinal-bar'),crIn=row.querySelector('.span-compression-rebar-required'),slIn=row.querySelector('.span-support-left'),srIn=row.querySelector('.span-support-right'),clIn=row.querySelector('.span-clear-length'),removeBtn=row.querySelector('.span-remove'),addRegionBtn=row.querySelector('.btn-add-region'),regenBtn=row.querySelector('.btn-regenerate-regions'),regionList=row.querySelector('.region-list'),summaryTitle=row.querySelector('.span-summary-title'),summaryMeta=row.querySelector('.span-summary-meta');
idIn.value=initial.id||'';sIn.value=initial.seismic||'';gIn.value=initial.gravity||'';drIn.value=Number.isFinite(initial.d_ratio)?String(initial.d_ratio):'0.90';lbIn.value=initial.longitudinal_bar||document.getElementById('db_bar').value||'#6';crIn.checked=initial.compression_rebar_required===undefined?document.getElementById('compression_rebar_required').checked:Boolean(initial.compression_rebar_required);slIn.value=Number.isFinite(initial.support_left_mm)?String(initial.support_left_mm):'';srIn.value=Number.isFinite(initial.support_right_mm)?String(initial.support_right_mm):'';clIn.value=Number.isFinite(initial.clear_length_mm)?String(initial.clear_length_mm):'';
const updateSummary=()=>{const id=(idIn.value||'').trim()||'sin ID',s=(sIn.value||'').trim()||'?',g=(gIn.value||'').trim()||'?',dr=(drIn.value||'').trim()||'0.90',lb=(lbIn.value||'').trim()||'#6',cr=crIn.checked?'sí':'no',sl=(slIn.value||'').trim()||'0',sr=(srIn.value||'').trim()||'0',cl=(clIn.value||'').trim();summaryTitle.textContent=`Vano ${id}`;summaryMeta.textContent=`Sismo:${s} | Gravedad:${g} | d/h:${dr} | Barra:${lb} | Compresión requerida:${cr} | Ap.izq:${sl} mm | Ap.der:${sr} mm | L libre real:${cl||'modelo'} mm`};
[idIn,sIn,gIn,drIn,lbIn,crIn,slIn,srIn,clIn].forEach(input=>input.addEventListener('input',updateSummary));
...
const idIn=row.querySelector('.span-id'),sIn=row.querySelector('.span-seismic'),gIn=row.querySelector('.span-gravity'),drIn=row.querySelector('.span-d-ratio'),lbIn=row.querySelector('.span-longitudinal-bar'),crIn=row.querySelector('.span-compression-rebar-required'),slIn=row.querySelector('.span-support-left'),srIn=row.querySelector('.span-support-right'),clIn=row.querySelector('.span-clear-length');
...
if(enableSpanLayout.checked){const spans=collectSpanLayoutStrict(errors);if(!errors.length){hiddenSpanLayoutInput.value=JSON.stringify(spans.map(s=>({id:s.id,seismic:s.seismic,gravity:s.gravity,d_ratio:s.d_ratio,longitudinal_bar:s.longitudinal_bar,compression_rebar_required:s.compression_rebar_required,support_left_mm:s.support_left_mm,support_right_mm:s.support_right_mm,clear_length_mm:s.clear_length_mm,regions:s.regions})));hiddenFramePairsInput.value=JSON.stringify(spans.map(s=>({id:s.id,seismic:s.seismic,gravity:s.gravity})))}}
...
addSpanBtn.addEventListener('click',()=>{spanLayoutList.appendChild(createSpanRow({}));currentPreviewPayload=null;refreshPreview()});enableSpanLayout.addEventListener('change',()=>{syncAdvancedSections();currentPreviewPayload=null;refreshPreview()});enableOptimizationOverrides.addEventListener('change',syncAdvancedSections);spanLayoutList.addEventListener('input',()=>{currentPreviewPayload=null;refreshPreview()});spanLayoutList.addEventListener('change',()=>{currentPreviewPayload=null;refreshPreview()});['beam_id','detailing','fc_mpa','fy_mpa','db_bar','compression_rebar_required'].forEach(id=>{const el=document.getElementById(id);if(el)el.addEventListener('input',()=>{currentPreviewPayload=null;refreshPreview()})});syncAdvancedSections();
