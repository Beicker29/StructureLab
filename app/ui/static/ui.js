...
const totalSpanMm=spans.reduce((a,s)=>a+(Number(s.length_mm)||0),0),totalSupportMm=spans.reduce((a,s)=>a+(Number(s.support_right_mm)||0),0),maxHeight=spans.reduce((m,s)=>Math.max(m,Number(s.height_mm)||0),0),maxWidth=spans.reduce((m,s)=>Math.max(m,Number(s.width_mm)||0),0);return{totalSpanMm,totalSupportMm,totalSystemMm:totalSpanMm+totalSupportMm,totalSpanEstimated:spans.some(s=>!!s.length_estimated),beamDepthMm:maxHeight||defaultHeight,beamWidthMm:maxWidth||defaultWidth,beamId:(document.getElementById('beam_id').value||'B1').trim()||'B1',detailing:(document.getElementById('detailing').value||'').trim(),fcMpa:parseNum(document.getElementById('fc_mpa').value)||28,fyMpa:parseNum(document.getElementById('fy_mpa').value)||420,spans,dataSource:'fallback'}
}
...
const transOptionsSource=(Array.isArray(rg.transverse_options)?rg.transverse_options:[]).map(opt=>({label:String(opt&&opt.label||'').trim(),weight_kg:Number.isFinite(Number(opt&&opt.weight_kg))?Number(opt.weight_kg):0,stirrup_count:Number.isFinite(Number(opt&&opt.stirrup_count))?Number(opt.stirrup_count):null,stirrup_unit_weight_kg:Number.isFinite(Number(opt&&opt.stirrup_unit_weight_kg))?Number(opt.stirrup_unit_weight_kg):null,})).filter(opt=>opt.label);
