...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box} html,body{height:100%}
    ...
    .brand{border:1px solid rgba(136,167,196,.26);background:rgba(11,28,45,.35);border-radius:12px;padding:11px;display:flex;align-items:center;gap:9px}
    .brand-mark{width:28px;height:28px;border-radius:7px;background:linear-gradient(165deg,#6f8fc5 0%,#4267a5 45%,#2d4f86 100%);border:1px solid rgba(170,194,225,.45);position:relative;box-shadow:inset 0 0 0 1px rgba(214,229,246,.18)}
    .brand-mark:before,.brand-mark:after{content:"";position:absolute;background:rgba(231,240,250,.82)}
    .brand-mark:before{width:14px;height:3px;left:6px;top:8px;transform:skew(-30deg)}
    .brand-mark:after{width:3px;height:14px;left:13px;top:8px;transform:skewY(-30deg)}
    .brand-wordmark{display:flex;align-items:baseline;gap:2px;font-size:1.1rem;font-weight:760;letter-spacing:.25px;line-height:1}
    .brand-strong{color:#f3f8ff}
    .brand-lab{color:#4f8dff}
    ...
    .artifact-link{color:#174e80;text-decoration:none;font-weight:620}
...
<body>
  <div class="app">
    <aside class="sidebar">
      <div class="brand"><span class="brand-mark" aria-hidden="true"></span><div class="brand-wordmark"><span class="brand-strong">STRUCTURE</span><span class="brand-lab">LAB</span></div></div>
    ...
    <main class="main">
      ...
      <form id="job-form" novalidate>
        ...
        <section id="view-reports" class="view">
          <div class="grid g2"><section class="card"><h2>Export y accesos rapidos</h2><p class="note">Enlaces de estado y case generado.</p><div id="results-links" class="links"></div></section><section class="card"><h2>Artefactos detectados</h2><ul id="artifacts-list" class="list"></ul></section></div>
        </section>
      </form>
    </main>
  </div>
  ...
  <script>
    ...
    let pollTimer=null,isSubmitting=false,currentJobId=null,currentPreviewPayload=null;
    ...
    function _fallbackSpanCount(){if(enableFramePairs.checked){const n=framePairsList.querySelectorAll(".pair-row").length;return n>0?n:1}return 1}
    function collectFallback(){const r=clamp(parseNum(document.getElementById("region_c_ratio").value)||0.2,0.05,0.45),count=_fallbackSpanCount(),spans=[];for(let i=0;i<count;i++){const regions=[{region_id:"R1",type:"C",length_mm:Math.round(6000*r),length_estimated:true,spacing_mm:100,spacing_estimated:true,transverse_label:"C @ 100 mm est.",longitudinal_label:"long. n/d"},{region_id:"R2",type:"NC",length_mm:Math.round(6000*(1-2*r)),length_estimated:true,spacing_mm:200,spacing_estimated:true,transverse_label:"NC @ 200 mm est.",longitudinal_label:"long. n/d"},{region_id:"R3",type:"C",length_mm:Math.round(6000*r),length_estimated:true,spacing_mm:100,spacing_estimated:true,transverse_label:"C @ 100 mm est.",longitudinal_label:"long. n/d"}];spans.push({span_id:`S${i+1}`,length_mm:6000,length_estimated:true,regions})}return{totalSpanMm:6000*count,totalSpanEstimated:true,beamDepthMm:parseNum(document.getElementById("height_mm").value)||600,beamWidthMm:parseNum(document.getElementById("width_mm").value)||300,beamId:(document.getElementById("beam_id").value||"B1").trim()||"B1",detailing:(document.getElementById("detailing").value||"DMO").trim()||"DMO",fcMpa:parseNum(document.getElementById("fc_mpa").value)||28,fyMpa:parseNum(document.getElementById("fy_mpa").value)||420,spans,dataSource:"fallback"}}
    function buildBeamData(source){const fb=collectFallback(),pp=source&&source.previewPayload;if(!pp||!Array.isArray(pp.spans)||!pp.spans.length)return fb;const beam=pp.beam||{},spans=pp.spans.map((sp,i)=>({span_id:sp.span_id||`S${i+1}`,length_mm:parseNum(sp.length_mm)||6000,length_estimated:!!sp.length_estimated,regions:Array.isArray(sp.regions)?sp.regions.map((rg,j)=>({region_id:rg.region_id||`R${j+1}`,type:String(rg.type||"NC").toUpperCase(),length_mm:parseNum(rg.length_mm)||1,length_estimated:!!rg.length_estimated,spacing_mm:parseNum(rg.spacing_mm)||defaultSpacing(rg.type),spacing_estimated:!!rg.spacing_estimated,transverse_label:rg.transverse_label||`${String(rg.type||"NC").toUpperCase()} @ ${parseNum(rg.spacing_mm)||defaultSpacing(rg.type)} mm`,longitudinal_label:rg.longitudinal_label||"long. n/d"})):[]}));const total=spans.reduce((acc,sp)=>acc+(parseNum(sp.length_mm)||0),0);return{totalSpanMm:parseNum(pp.total_span_mm)||total||fb.totalSpanMm,totalSpanEstimated:!!pp.total_span_estimated,beamDepthMm:parseNum(beam.height_mm)||fb.beamDepthMm,beamWidthMm:parseNum(beam.width_mm)||fb.beamWidthMm,beamId:beam.beam_id||fb.beamId,detailing:beam.detailing||fb.detailing,fcMpa:parseNum(beam.fc_mpa)||fb.fcMpa,fyMpa:parseNum(beam.fy_mpa)||fb.fyMpa,spans,dataSource:"preview"}}
    function buildBeamSvg(d){const W=1400,H=430,x0=80,y=130,h=90,bw=W-2*x0,dy=68,by=y+h,total=Math.max(d.totalSpanMm||6000,1000),spans=Array.isArray(d.spans)&&d.spans.length?d.spans:[];let zoneRects="",zoneLines="",stirrup="",labels="",spanMarkers="",spanLabels="";let cursor=x0;spans.forEach((sp,si)=>{const spanLen=Math.max(parseNum(sp.length_mm)||1,1),sw=(spanLen/total)*bw;spanMarkers+=`<line x1="${cursor.toFixed(2)}" y1="${y}" x2="${cursor.toFixed(2)}" y2="${by}" stroke="#254b70" stroke-width="${si===0?1.4:2}" stroke-dasharray="${si===0?"0":"5 4"}" />`;spanLabels+=`<text x="${(cursor+sw/2).toFixed(2)}" y="100" text-anchor="middle" font-size="12" fill="#1b3552" font-weight="620">${sp.span_id||`S${si+1}`}</text>`;let rc=cursor;(Array.isArray(sp.regions)?sp.regions:[]).forEach((rg,ri)=>{const rl=Math.max(parseNum(rg.length_mm)||1,1),rw=(rl/spanLen)*sw,rx1=rc,rx2=rc+rw,fill=(si+ri)%2?"rgba(36,92,140,0.07)":"rgba(20,65,106,0.10)";zoneRects+=`<rect x="${rx1.toFixed(2)}" y="${y}" width="${Math.max(rw,2).toFixed(2)}" height="${h}" fill="${fill}" />`;zoneLines+=`<line x1="${rx1.toFixed(2)}" y1="${y}" x2="${rx1.toFixed(2)}" y2="${by}" stroke="#28567f" stroke-width="1.1" stroke-dasharray="4 3" />`;const step=clamp((Math.max(parseNum(rg.spacing_mm)||defaultSpacing(rg.type),1)/total)*bw,8,54);for(let xx=rx1+5;xx<rx2-5;xx+=step){stirrup+=`<path d="M ${xx.toFixed(2)} ${y+6} L ${xx.toFixed(2)} ${by-6}" stroke="#2f4f6f" stroke-width="1.05" fill="none" />`}const cx=rx1+rw/2,lenTxt=Math.round(rl),line1=`${sp.span_id||`S${si+1}`}-${rg.region_id||`R${ri+1}`} (L=${lenTxt} mm${rg.length_estimated?" est.":""})`,line2=`${rg.transverse_label||`@${Math.round(parseNum(rg.spacing_mm)||defaultSpacing(rg.type))} mm`}`,line3=`${rg.longitudinal_label||"long. n/d"}`;labels+=`<text x="${cx.toFixed(2)}" y="278" text-anchor="middle" font-size="12" fill="#1a2f47" font-weight="620"><tspan x="${cx.toFixed(2)}" dy="0">${line1}</tspan><tspan x="${cx.toFixed(2)}" dy="14">${line2}</tspan><tspan x="${cx.toFixed(2)}" dy="14">${line3}</tspan></text>`;rc=rx2});cursor+=sw});spanMarkers+=`<line x1="${(x0+bw).toFixed(2)}" y1="${y}" x2="${(x0+bw).toFixed(2)}" y2="${by}" stroke="#254b70" stroke-width="1.4" stroke-dasharray="5 4" />`;const spanTxt=`${Math.round(total)} mm${d.totalSpanEstimated?" (estimado)":""}`;return `<svg id="beam-elevation" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Beam elevation"><defs><marker id="arrow" viewBox="0 0 10 10" refX="5" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z" fill="#1e334a" /></marker></defs><rect x="12" y="12" width="${W-24}" height="${H-24}" rx="8" fill="#f8fbff" stroke="#d0dcea" /><line x1="${x0}" y1="${dy}" x2="${x0+bw}" y2="${dy}" stroke="#1e334a" stroke-width="1.4" marker-start="url(#arrow)" marker-end="url(#arrow)" /><text x="${x0+bw/2}" y="${dy-10}" text-anchor="middle" font-size="16" fill="#16304a" font-weight="700">Total span: ${spanTxt}</text><line x1="${x0-36}" y1="${y}" x2="${x0-36}" y2="${by}" stroke="#1e334a" stroke-width="1.2" marker-start="url(#arrow)" marker-end="url(#arrow)" /><text x="${x0-48}" y="${y+h/2}" transform="rotate(-90 ${x0-48} ${y+h/2})" text-anchor="middle" font-size="13" fill="#183753" font-weight="620">h = ${Math.round(d.beamDepthMm||600)} mm</text><polygon points="${x0-14},${by+24} ${x0+14},${by+24} ${x0},${by+2}" fill="#5f748d" /><polygon points="${x0+bw-14},${by+24} ${x0+bw+14},${by+24} ${x0+bw},${by+2}" fill="#5f748d" />${zoneRects}<rect x="${x0}" y="${y}" width="${bw}" height="${h}" fill="none" stroke="#2b4f73" stroke-width="2.1" rx="4" />${spanMarkers}${zoneLines}${stirrup}${spanLabels}${labels}</svg>`}
    function renderBeam(container,data){container.innerHTML=buildBeamSvg(data);beamElevationLegend.textContent=data.dataSource==="preview"?"Dibujo alimentado por datos de job/optimizacion (incluye vanos multiples y arreglos por region).":"Se usan datos de fallback (estimados) hasta obtener preview del job."}
    function renderSummary(data,st){summaryListEl.innerHTML=[`<li><strong>Beam:</strong> ${data.beamId} | Detailing ${data.detailing}</li>`,`<li><strong>Material:</strong> f'c ${Math.round(data.fcMpa)} MPa | fy ${Math.round(data.fyMpa)} MPa</li>`,`<li><strong>Seccion:</strong> b=${Math.round(data.beamWidthMm)} mm | h=${Math.round(data.beamDepthMm)} mm</li>`,`<li><strong>Vanos:</strong> ${Array.isArray(data.spans)?data.spans.length:0}</li>`,`<li><strong>Total span:</strong> ${Math.round(data.totalSpanMm)} mm${data.totalSpanEstimated?" (estimado)":""}</li>`,`<li><strong>Job status:</strong> ${(st&&st.status)||"sin job"}</li>`].join("")}
    function renderChecks(st){const rows=[];const s=st&&st.status?st.status:"idle";if(s==="completed")rows.push({c:"ok",m:"Job completado correctamente."});else if(s==="failed")rows.push({c:"err",m:"Job finalizo en estado failed."});else if(s==="running"||s==="queued")rows.push({c:"warn",m:`Job en progreso: ${s}.`});else rows.push({c:"warn",m:"Aun no se ha ejecutado un job en esta sesion."});rows.push({c:"ok",m:"Contratos API preservados (/v1/jobs*)."});rows.push({c:"ok",m:"Preview SVG soporta vanos multiples y etiqueta transversal/longitudinal."});checksListEl.innerHTML=rows.map(r=>`<li class="${r.c}">${r.m}</li>`).join("")}
    ...
    addPairBtn.addEventListener("click",()=>{framePairsList.appendChild(createPairRow({}));currentPreviewPayload=null;refreshPreview()});enableFramePairs.addEventListener("change",()=>{syncAdvancedSections();currentPreviewPayload=null;refreshPreview()});enableOptimizationOverrides.addEventListener("change",syncAdvancedSections);syncAdvancedSections();
    ["beam_id","detailing","fc_mpa","fy_mpa","width_mm","height_mm","region_c_ratio"].forEach(id=>{const el=document.getElementById(id);if(el)el.addEventListener("input",()=>{currentPreviewPayload=null;refreshPreview()})});
