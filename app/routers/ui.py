...
.span-head{display:grid;grid-template-columns:repeat(6,minmax(0,1fr)) auto;gap:8px;align-items:end}
...
<div class="field"><label for="units_rebar_per_length">Unidad VRebar/TTrnRebar</label><select id="units_rebar_per_length" name="units_rebar_per_length"><option value="mm2/m" selected>mm2/m</option><option value="cm2/m">cm2/m</option></select></div><div class="field"><label for="geometry_units">Unidad de Width/Depth (Geometria)</label><select id="geometry_units" name="geometry_units"><option value="mm" selected>mm</option><option value="cm">cm</option><option value="m">m</option><option value="in">in</option></select><p class="help">Se aplica a columnas Width y Depth del archivo Geometria.</p></div>
</div></section>
...
<input type="hidden" id="min_branches_c" name="min_branches_c" value="4" /><input type="hidden" id="min_branches_nc" name="min_branches_nc" value="2" /><input type="hidden" id="region_c_ratio" name="region_c_ratio" value="0.2" /><input type="hidden" id="width_mm" name="width_mm" value="300" /><input type="hidden" id="height_mm" name="height_mm" value="600" /><input type="hidden" id="d_mm" name="d_mm" value="540" /><input type="hidden" id="d_ratio_default" name="d_ratio_default" value="0.9" />
...
const idIn=row.querySelector('.span-id'),sIn=row.querySelector('.span-seismic'),gIn=row.querySelector('.span-gravity'),drIn=row.querySelector('.span-d-ratio'),slIn=row.querySelector('.span-support-left'),srIn=row.querySelector('.span-support-right'),removeBtn=row.querySelector('.span-remove'),addRegionBtn=row.querySelector('.btn-add-region'),regenBtn=row.querySelector('.btn-regenerate-regions'),regionList=row.querySelector('.region-list'),summaryTitle=row.querySelector('.span-summary-title'),summaryMeta=row.querySelector('.span-summary-meta');
...
const updateSummary=()=>{const id=(idIn.value||'').trim()||'sin ID',s=(sIn.value||'').trim()||'?',g=(gIn.value||'').trim()||'?',dr=(drIn.value||'').trim()||'0.90',sl=(slIn.value||'').trim()||'0',sr=(srIn.value||'').trim()||'0';summaryTitle.textContent=`Vano ${id}`;summaryMeta.textContent=`Sismo:${s} | Gravedad:${g} | d/h:${dr} | Ap.izq:${sl} mm | Ap.der:${sr} mm`};
[idIn,sIn,gIn,drIn,slIn,srIn].forEach(input=>input.addEventListener('input',updateSummary));
...
const idIn=row.querySelector('.span-id'),sIn=row.querySelector('.span-seismic'),gIn=row.querySelector('.span-gravity'),drIn=row.querySelector('.span-d-ratio'),slIn=row.querySelector('.span-support-left'),srIn=row.querySelector('.span-support-right');
...
function collectFallback(){const defaultRatio=DEFAULT_C_RATIO,dbBar=(document.getElementById('db_bar').value||'#6').trim()||'#6',layout=collectSpanLayoutPreview(),defaultWidth=parseNum(document.getElementById('width_mm').value)||300,defaultHeight=parseNum(document.getElementById('height_mm').value)||600,spans=[];if(layout.length){layout.forEach((span,i)=>{const spanWidth=Math.max(1,Number(span.width_mm)||defaultWidth),spanHeight=Math.max(1,Number(span.height_mm)||defaultHeight),spanDRatio=Math.min(1,Math.max(0.01,Number(span.d_ratio)||0.9));const regions=(span.regions||[]).map((region,j)=>{const lengthMm=Math.max(1,Math.round(6000*(region.to-region.from))),labels=fallbackRegionLabels(region,dbBar);return{region_id:region.id||`R${j+1}`,type:String(region.type||'NC').toUpperCase(),from:region.from,to:region.to,length_mm:lengthMm,length_estimated:true,spacing_mm:Number(region.spacing_mm)||defaultSpacing(region.type),spacing_estimated:true,transverse_label:labels.transverse,longitudinal_label:labels.longitudinal}});const spanLength=regions.reduce((a,r)=>a+(Number(r.length_mm)||0),0)||6000;spans.push({span_id:span.id||`S${i+1}`,seismic:span.seismic,gravity:span.gravity,d_ratio:spanDRatio,d_mm:Math.round(spanHeight*spanDRatio),width_mm:Math.round(spanWidth),height_mm:Math.round(spanHeight),support_left_mm:Math.max(0,Math.round(Number(span.support_left_mm)||0)),support_right_mm:Math.max(0,Math.round(Number(span.support_right_mm)||0)),length_mm:spanLength,length_estimated:true,regions})})}
if(!spans.length){const base=defaultRegionTemplate(defaultRatio).map(region=>{const labels=fallbackRegionLabels(region,dbBar);return{region_id:region.id,type:region.type,from:region.from,to:region.to,length_mm:Math.max(1,Math.round(6000*(region.to-region.from))),length_estimated:true,spacing_mm:region.spacing_mm,spacing_estimated:true,transverse_label:labels.transverse,longitudinal_label:labels.longitudinal}});spans.push({span_id:'S1',seismic:'S1',gravity:'S1',d_ratio:0.9,d_mm:Math.round(defaultHeight*0.9),width_mm:Math.round(defaultWidth),height_mm:Math.round(defaultHeight),support_left_mm:0,support_right_mm:0,length_mm:6000,length_estimated:true,regions:base})}
const totalSpanMm=spans.reduce((a,s)=>a+(Number(s.length_mm)||0),0),totalSupportMm=spans.reduce((a,s)=>a+(Number(s.support_right_mm)||0),0),maxHeight=spans.reduce((m,s)=>Math.max(m,Number(s.height_mm)||0),0),maxWidth=spans.reduce((m,s)=>Math.max(m,Number(s.width_mm)||0),0);return{totalSpanMm,totalSupportMm,totalSystemMm:totalSpanMm+totalSupportMm,totalSpanEstimated:true,beamDepthMm:maxHeight||defaultHeight,beamWidthMm:maxWidth||defaultWidth,beamId:(document.getElementById('beam_id').value||'B1').trim()||'B1',detailing:(document.getElementById('detailing').value||'DMO').trim()||'DMO',fcMpa:parseNum(document.getElementById('fc_mpa').value)||28,fyMpa:parseNum(document.getElementById('fy_mpa').value)||420,spans,dataSource:'fallback'}}
function buildBeamData(source){const fb=collectFallback(),pp=source&&source.previewPayload;if(!pp||!Array.isArray(pp.spans)||!pp.spans.length)return fb;const beam=pp.beam||{},spans=pp.spans.map((sp,i)=>({span_id:sp.span_id||`S${i+1}`,seismic:sp.seismic||'',gravity:sp.gravity||'',d_mm:parseNum(sp.d_mm),width_mm:parseNum(sp.width_mm),height_mm:parseNum(sp.height_mm),support_left_mm:parseNum(sp.support_left_mm)||0,support_right_mm:parseNum(sp.support_right_mm)||0,length_mm:parseNum(sp.length_mm)||6000,length_estimated:!!sp.length_estimated,regions:Array.isArray(sp.regions)?sp.regions.map((rg,j)=>({region_id:rg.region_id||`R${j+1}`,type:String(rg.type||'NC').toUpperCase(),from:parseNum(rg.from),to:parseNum(rg.to),length_mm:parseNum(rg.length_mm)||1,length_estimated:!!rg.length_estimated,spacing_mm:parseNum(rg.spacing_mm)||defaultSpacing(rg.type),spacing_estimated:!!rg.spacing_estimated,transverse_label:rg.transverse_label||`${String(rg.type||'NC').toUpperCase()} @ ${parseNum(rg.spacing_mm)||defaultSpacing(rg.type)} mm`,longitudinal_label:rg.longitudinal_label||'long. n/d'})):[]}));const total=spans.reduce((a,s)=>a+(parseNum(s.length_mm)||0),0),supp=spans.reduce((a,s)=>a+(parseNum(s.support_right_mm)||0),0),maxHeight=spans.reduce((m,s)=>Math.max(m,parseNum(s.height_mm)||0),0),maxWidth=spans.reduce((m,s)=>Math.max(m,parseNum(s.width_mm)||0),0);return{totalSpanMm:parseNum(pp.total_span_mm)||total||fb.totalSpanMm,totalSupportMm:parseNum(pp.total_support_mm)||supp,totalSystemMm:parseNum(pp.total_system_mm)||((parseNum(pp.total_span_mm)||total||fb.totalSpanMm)+(parseNum(pp.total_support_mm)||supp)),totalSpanEstimated:!!pp.total_span_estimated,beamDepthMm:parseNum(beam.height_mm)||maxHeight||fb.beamDepthMm,beamWidthMm:parseNum(beam.width_mm)||maxWidth||fb.beamWidthMm,beamId:beam.beam_id||fb.beamId,detailing:beam.detailing||fb.detailing,fcMpa:parseNum(beam.fc_mpa)||fb.fcMpa,fyMpa:parseNum(beam.fy_mpa)||fb.fyMpa,spans,dataSource:'preview'}}
